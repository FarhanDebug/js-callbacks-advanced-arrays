...
const fullName = runners.forEach(name => (first_name, last_name));
  console.log(fullName)
...
const allCaps = runners.filter(item => item.first_name.toUpperCase);
...
let ticketPriceTotal = runners.reduce((total, ticket) => {
...
const totalDoners = runners.reduce(doner(accumulator, currentValue){
    console.log(`I am the accumulator ${accumulator}`)
    console.log(`I am the current Value ${currentValue.donation}`)
}, 0)
